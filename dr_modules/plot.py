    ...
    def response(self,title,num=-1,fs=5,density=True,dpi=100,logscale=False,figsize=1.6):
        ...
        for i in range(len(self.load)):
            ...
            if(len(y.shape)==2):
                y=y[:,0]
            p=self.load[i]['p'][:,0]
            B.hist(p[np.where(y==1)[0]],range=[0,1],bins=30,alpha=0.5,label=self.label[i][0],density=density,color='C{}'.format(count*2))
            B.hist(p[np.where(y==0)[0]],range=[0,1],bins=30,alpha=0.5,label=self.label[i][1],density=density,color='C{}'.format(count*2+1))
            count+=1
